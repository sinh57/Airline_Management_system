...
if(isset($_GET['id'])){
    ...
    // Check if flight exists and get differently-abled price
    $stmt = mysqli_prepare($con, "SELECT FLIGHT_CODE, PRICE_DIFFERENTLYABLED FROM flight WHERE FLIGHT_CODE = ?");
    ...
    $price = $row['PRICE_DIFFERENTLYABLED'];
    mysqli_stmt_close($stmt);
    
    // Store flight selection in session
    $_SESSION['selected_flight'] = $id;
    $_SESSION['ticket_type'] = 'DIFFERENTLY ABLED';
